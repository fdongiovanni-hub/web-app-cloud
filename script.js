...
// --- Scia di scintille dietro al cursore ---
(function cursorSparkles() {
  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (prefersReducedMotion) return; // rispetta chi ha disattivato le animazioni

  let lastSpawn = 0;
  window.addEventListener('mousemove', (e) => {
    const now = Date.now();
    if (now - lastSpawn < 45) return; // non crearne troppe, una ogni ~45ms
    lastSpawn = now;
